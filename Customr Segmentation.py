...
Rtl_data = pd.read_csv('RetailData.csv', encoding = 'unicode_escape')  #to avoid any error from any invalid value or character
Rtl_data.head()
...
RFMScores.rename(columns={'InvoiceDate': 'Recency', 
                         'InvoiceNo': 'Frequency', 
                         'TotalAmount': 'Monetary'}, inplace=True) #we don't have any new copy of data
...
Score_cuts = pd.qcut(RFMScores.RFMScore, q = 4, labels = Loyalty_Level) #quantile-based discretization function used to divide up the data to into equal sized bins
...
sum_of_sq_dist = {}  #sum of the square distances of samples to the nearset cluster center 
for k in range(1,15):
